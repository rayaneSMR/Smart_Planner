...
class TaskService {
  ...
  Future<void> updateTask(int index, Task task) async {
    ...
    final previousTask = _box.get(key);
    if (previousTask != null) {
      await _notificationService.cancelNotifications(previousTask);
    }
  ...
  Future<void> updateTaskEventId(Task task, String eventId) async {
    final idx = _box.values.toList().indexWhere(
      (t) =>
          t.title == task.title &&
  ...
  Future<void> rescheduleAllNotifications() async {
    ...
    for (final task in _tasks) {
      await _notificationService.scheduleDeadlineNotification(task);
      await _notificationService.scheduleDeadlineReachedNotification(
        task,
